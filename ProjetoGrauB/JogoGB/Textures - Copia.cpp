...
int setupShader();
int setupGeometry();
Sprite initializeSprite(GLuint texID, vec3 dimensions, vec3 position, int nAnimations=1, int nFrames=1, float vel = 0.3, float angle=0.0);
...
//#endif
	...
	Sprite background, character, snowball;
	...
	character = initializeSprite(texID, vec3(3*imgWidth, 3*imgHeight, 1.0), vec3(400, 100, 0), 3, 4);

	// Carregando a textura do background e armazenando seu id
	texID = loadTexture("../Textures/bgsketch.png", imgWidth, imgHeight);
	background = initializeSprite(texID, vec3(imgWidth, imgHeight, 1.0), vec3(400, 300, 0));

	// Carregando a textura da bola de neve e armazenando seu id
	texID = loadTexture("../Textures/snowball.png", imgWidth, imgHeight);
	snowball = initializeSprite(texID, vec3(3*imgWidth, 3*imgHeight, 1.0), vec3(400, 400, 0));
